...
/**
 * swipe stack which like Tinder and TanTan
 */
public class SwipeCards extends ViewGroup {
    ...
    private void readAttributes(AttributeSet attributeSet) {
        TypedArray attrs = getContext().obtainStyledAttributes(attributeSet, R.styleable.UST_cards);

        try {
            mAllowedSwipeDirections =
                    attrs.getInt(R.styleable.UST_cards_UST_allowed_swipe_directions,
                            SWIPE_DIRECTION_BOTH);
            mAnimationDuration =
                    attrs.getInt(R.styleable.UST_cards_UST_animation_duration,
                            DEFAULT_ANIMATION_DURATION);
            mNumberOfStackedViews =
                    attrs.getInt(R.styleable.UST_cards_UST_stack_size, DEFAULT_STACK_SIZE);
            mViewSpacing =
                    attrs.getDimensionPixelSize(R.styleable.UST_cards_UST_stack_spacing,
                            getResources().getDimensionPixelSize(R.dimen.default_stack_spacing));
            mViewRotation =
                    attrs.getInt(R.styleable.UST_cards_UST_stack_rotation, DEFAULT_STACK_ROTATION);
            mSwipeRotation =
                    attrs.getFloat(R.styleable.UST_cards_UST_swipe_rotation, DEFAULT_SWIPE_ROTATION);
            mSwipeOpacity =
                    attrs.getFloat(R.styleable.UST_cards_UST_swipe_opacity, DEFAULT_SWIPE_OPACITY);
            mScaleFactor =
                    attrs.getFloat(R.styleable.UST_cards_UST_scale_factor, DEFAULT_SCALE_FACTOR);
            mDisableHwAcceleration =
                    attrs.getBoolean(R.styleable.UST_cards_UST_disable_hw_acceleration,
    ...
    private void initialize() {
        ...
        mSwipeCardsHelper = new SwipeCardsHelper(this);
        mSwipeCardsHelper.setAnimationDuration(mAnimationDuration);
        mSwipeCardsHelper.setRotation(mSwipeRotation);
        mSwipeCardsHelper.setOpacityEnd(mSwipeOpacity);
    ...
    private void removeTopView() {
        ...
        if (getChildCount() == 0) {
            if (mListener != null) mListener.onCardsEmpty();
        }
    }
    ...
    public void swipeTopViewToRight() {
        ...
        mSwipeCardsHelper.swipeViewToRight();
    }
    ...
    public void swipeTopViewToLeft() {
        if (getChildCount() == 0) return;
        mSwipeCardsHelper.swipeViewToLeft();
    }
